...
public enum SigningFailureGuidance {
    ...
    public static func whatToDo(_ code: SigningFailureCode) -> String {
        ...
        case .signingIdentityUnavailable:
            return """
                워커 머신의 키체인에서 Developer ID 인증서를 확인하세요. 관리 > 개발자 포털에서 \
                만료일을 볼 수 있습니다. 인증서를 새로 받았다면 워커 설정의 서명 identity 이름도 \
                함께 맞춰야 합니다. 다시 올려도 같은 실패가 납니다.
                """
        case .codesignFailed:
            return """
                번들 안에 codesign 이 받아들이지 못하는 것이 있습니다. 로그의 마지막 줄이 어느 \
                파일에서 걸렸는지 알려줍니다. 리소스 포크나 확장 속성이 붙은 파일, 이미 서명이 \
                깨진 프레임워크가 흔한 원인입니다.
                """
        ...
        case .unsignedCodeRemains:
            return """
                번들 안에 우리가 서명하지 못한 코드가 남았습니다. 앱 쪽 문제가 아니라 워커의 \
                서명 대상 선정이 그 파일을 놓친 것일 수 있으니, 로그에 적힌 경로와 함께 알려주세요.
                """
        case .notarizationRejected:
            return """
                Apple 이 내용을 보고 거절했습니다. 로그 끝에 붙은 공증 로그가 어느 파일의 무엇이 \
                문제인지 알려줍니다. 대개 Hardened Runtime 이 없거나 서명이 빠진 바이너리입니다. \
                고쳐서 다시 올려야 합니다.
                """
        case .transferFailed:
            return """
                아티팩트를 주고받지 못했습니다. 서버가 자동으로 다시 시도합니다. 계속 실패하면 \
                오브젝트 스토리지와 워커 머신의 네트워크를 확인하세요.
                """
        case .appleServiceUnavailable:
            return """
                공증 서비스에 닿지 못했습니다. 서버가 자동으로 다시 시도합니다. 계속 실패하면 \
                Apple 시스템 상태와 워커의 공증 자격증명(notary profile)을 확인하세요.
                """
        case .timedOut:
            return """
                명령이 제한 시간 안에 끝나지 않았습니다. 서버가 자동으로 다시 시도합니다. 앱이 \
                아주 크거나 공증이 밀리면 일어납니다.
                """
        case .unknown:
            return """
                아직 갈래로 나누지 못한 실패입니다. 자동으로 다시 시도하지 않습니다. 아래 로그를 \
                그대로 전달해 주세요. 같은 실패가 반복되면 갈래를 하나 더 만들어야 한다는 뜻입니다.
                """
        }
    }
}
