...
extension StoreAppSettings {
    ...
    /// 빌드할 것이 아무것도 없을 때 화면에 적을 말.
    ///
    /// **거의 나오지 않는다.** 서버 이미지에 스토어 앱이 함께 들어 있어서
    /// (ADR-0048) 보통은 올리지 않아도 빌드할 것이 있다. 이 줄이 보이는 것은
    /// 그 번들이 없는 이미지로 돌 때뿐이다. 로컬에서 `docker build` 를 그냥 돌렸
    /// 거나, macOS 잡이 실패한 릴리스가 그렇다.
    ///
    ...
    public static let noBundleBlockers = [
        "이 서버 이미지에 스토어 앱이 들어 있지 않습니다. 제품 릴리스의 번들을 올려주세요."
    ]
