...
| SMTP 계정 | 배포 플랫폼 시크릿 `SMTP_USERNAME` / `SMTP_PASSWORD` | 릴레이 정책에 따름 | | **위와 같음.** 메일로 받기로 한 사람에게만 안 감 |

마지막 셋은 없어도 서버가 뜹니다. 그래서 끊겨도 한동안 모릅니다. 알림 쪽이 특히
그런데, 알림이 안 오는 것과 실패가 없는 것이 구분되지 않습니다. Slack 앱을
워크스페이스에서 지우거나 토큰을 새로 발급하면 옛 토큰이 죽는다는 것을 알아두세요.
