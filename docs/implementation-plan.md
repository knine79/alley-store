...
| Sparkle 서명키 교체 절차 | appcast 를 실제로 쓰기 전 | 키를 잃으면 이미 배포된 앱들이 업데이트를 못 받는다. 앱에 박힌 `SUPublicEDKey` 를 바꾸려면 새 빌드를 다른 경로로 배포해야 하는데, 그 절차가 아무 데도 적혀 있지 않다 ([ADR-0017](adr/0017-sparkle-feed-tokens.md)) |
