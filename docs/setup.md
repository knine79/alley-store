...
알림은 셋으로 나뉘고, 셋 다 같은 모양으로 고릅니다 (ADR-0059).

| 갈래 | 어디로 | 어디서 정하나 |
| --- | --- | --- |
| 앱 알림 | Slack 채널 **또는** 올릴 수 있는 사람들 | 앱 상세의 **알림** 에서 앱마다 |
| 운영 알림 | Slack 채널 **또는** 관리자들 | 관리 &gt; **알림** |
| 사람에게 갈 때 무엇으로 | Slack DM 또는 메일 | 각자 **내 알림** |

채널은 등록해 둔 웹훅으로 가고, 사람에게 갈 때는 각자 정한 수단으로 갑니다. 그래서
사람에게 보내려면 아래 둘 중 하나는 있어야 합니다.

지금 사람에게 나가는 것은 서명 실패와 피드백입니다. Slack 봇도 메일도 없으면 개별
전송을 고를 수 없고, 채널도 안 걸어뒀으면 그 알림이 아무 데도 가지 않습니다.

**둘 다 넣으면 받는 사람이 고릅니다** (ADR-0058). 각자 **내 알림** 에서 Slack DM 과
메일 중 하나를 고르고, 한쪽만 갖춘 스토어에서는 고를 것 없이 그쪽으로 갑니다. 같은
알림을 두 번 보내지 않습니다.
...
| 변수 | 값 |
| --- | --- |
| `SMTP_HOST` | 릴레이 호스트. **이것과 `SMTP_FROM` 이 둘 다 있어야 켜집니다** |
| `SMTP_FROM` | 보내는 주소. 릴레이가 허용하는 주소여야 합니다 |
| `SMTP_PORT` | 기본 587 |
| `SMTP_FROM_NAME` | 보내는 이름. 기본 `Alley` |
| `SMTP_USERNAME`, `SMTP_PASSWORD` | **둘 다 있을 때만** 인증합니다 |
| `SMTP_SECURE` | `starttls` (기본) / `ssl` / `none` |

사내 릴레이는 망 안에서 오는 것을 그냥 받는 경우가 많습니다. 그때는 사용자 이름과
비밀번호를 비웁니다. 빈 자격증명으로 로그인을 시도하면 오히려 거절당합니다.

모르는 `SMTP_SECURE` 값은 `starttls` 로 접습니다. 오타 하나로 비밀번호가 평문으로
나가는 일을 만들지 않으려는 것이라, 정말 평문으로 보내려면 `none` 이라고 적어야
합니다.

받는 사람의 주소는 **스토어 계정의 이메일**입니다. 따로 등록할 것이 없습니다.
...
릴리스 한 번을 내는 동안 자리를 여러 번 옮깁니다. 올리고, 콘솔을 새로고침하며
...
넣을 `SUPublicEDKey` 를 가져오고, 들어온 피드백을 읽습니다.

앱을 지우거나 사람을 떼는 것은 내주지 않습니다. 돌이킬 수 없는 일은 화면에서
사람이 합니다.
