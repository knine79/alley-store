...
| 받는 곳 | 무엇으로 | 어디서 정하나 |
| --- | --- | --- |
| 앱 채널 | Incoming Webhook, 메일 주소 | 앱 상세의 **알림** 에서 앱마다 |
| 사람 | 봇 토큰, 메일 | `SLACK_BOT_TOKEN` 과 `SMTP_*` 로 스토어 전체 |

**웹훅으로는 사람에게 못 보냅니다.** 만들 때 정한 채널 하나에 쓰는 것이라 받는
사람을 고를 수 없습니다. 서명 실패처럼 고칠 사람이 정해진 알림은 그 사람에게
닿아야 합니다.

지금 사람에게 나가는 것은 서명 실패와 피드백입니다. Slack 봇도 메일도 없으면 그
알림이 가지 않고, 올린 사람이 웹 콘솔에 다시 들어와야 실패를 압니다.
