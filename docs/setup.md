...
운영 레포의 `adopt.yml` 이 돌 때마다 새 베이스 번들을 만들어 올리고 빌드까지
시킵니다 ([ADR-0046](adr/0046-server-assembles-store-app.md)). 같은 버전이 이미
있으면 건너뜁니다. 3번에서 `ALLEY_OPERATOR_TOKEN` 을 넣어뒀다면 따로 할 일이
없습니다.

**남는 일은 하나입니다.** 관리 > 스토어 앱에서 이름·번들 ID·URL 스킴·아이콘을 정하고,
빌드가 `배포 준비됨` 이 되면 **출시** 를 누릅니다. 출시를 사람이 누르는 것은 일부러
그렇게 둔 것입니다. 구성원의 맥에 설치될 앱이라 CI 사고가 그대로 나가면 안 됩니다.

### 손으로 올리기

CI 를 아직 안 붙였거나 특정 번들을 콕 집어 내보내야 할 때 씁니다.

1. 제품 릴리스에서 `alley-store-app-unsigned.zip` 을 받습니다
2. **관리 > 스토어 앱** 에서 그것을 "베이스 번들" 로 올립니다
3. 같은 화면에서 이름·번들 ID·URL 스킴·아이콘을 정합니다
4. **빌드해서 올리기** 를 누릅니다. 서버가 번들을 만들고 서명 워커가 서명·공증합니다
5. 같은 화면의 빌드 표에서 **출시** 를 누릅니다
...
`scripts/build-store-app.sh` 는 남아 있지만 하는 일이 달라졌습니다. 운영 CI 가 이것으로
브랜딩 없는 베이스 번들을 만들고, 로컬에서 앱을 띄워볼 때도 씁니다. 조직의 이름과
아이콘을 입히는 것은 서버 몫입니다.
