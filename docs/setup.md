...
받는 사람은 **앱 화면의 "이 버전 받기"** 로 내려받습니다. 스토어 앱이 없는 사람이
스토어 앱을 받을 수 있는 유일한 자리라 여기만 열어둡니다. 다른 앱은 스토어 앱으로
받습니다.
...
**앱 이름에 한글을 써도 됩니다.** 다만 번들 안의 실행 파일 이름은 ASCII 로 바뀝니다.
비ASCII 문자가 들어가면 `codesign --verify --deep --strict` 가 그 번들을
`a sealed resource is missing or invalid` 로 거절하는데, **그 실패는 서명이 끝난
뒤에야 나오고** 문구만 보고 이름이 원인이라는 것을 알 수 없습니다. `.app` 폴더 이름과
Finder·Dock 에 보이는 이름은 적은 그대로입니다. 스크립트가 바꿨을 때는 시작 줄에
그 사실을 적습니다.
