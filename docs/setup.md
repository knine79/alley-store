...
1. 웹 콘솔의 **새 앱** 화면에서 앱 파일을 고릅니다. zip 이나 dmg 둘 다 됩니다.
   **zip 이면** 번들 ID·이름·버전·빌드 번호·최소 macOS 를 읽어서 채워줍니다. dmg 는
   브라우저가 열 수 없어 직접 적으셔야 합니다
2. Electron 처럼 JIT 를 쓰는 런타임을 품은 앱이면 entitlements plist 도 함께
   고릅니다 (아래 참고)
3. **등록** 을 누르면 앱 등록과 첫 버전 업로드가 함께 됩니다. 번들 ID 는 나중에
   못 바꿉니다
4. 미서명으로 올렸다면 워커가 가져가 서명·공증합니다. 상태가 `배포 준비됨` 이 되면
5. **출시** 를 누릅니다. 그때부터 스토어 앱 목록에 보입니다
...
이미 서명·공증을 마친 번들이면 워커가 그것을 알아보고 서명을 건너뛰므로 필요 없습니다
([ADR-0035](adr/0035-worker-decides-signing-state.md)). 워커가 다시 서명하게 되는
경우에도, 안 주면 붙어 있던 권한을 그대로 읽어 다시 붙입니다.
