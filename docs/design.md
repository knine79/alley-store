...
| 인증       | 표준 OIDC. 공급자는 설정으로 정한다 (ADR-0047). 허용 도메인은 서버 설정. 다운로드도 로그인 필수 (예외: 스토어 앱 자신을 받는 `/get`, ADR-0049) |
...
화면에서 할 수 있는 것은 대체로 이 토큰으로도 됩니다. **다만 지우지는 못합니다.**
`/api/v1` 밖에서는 인증하지 않고, 그 안에서도 `DELETE` 는 거절합니다. 90일이면
만료되고, 계정을 끊으면 함께 폐기됩니다
...
| 테이블              | 주요 필드                                                      |
| ---------------- | ---------------------------------------------------------- |
| `users`          | google_sub, email, name, avatar_url, role, deactivated_at  |
...
- **스토어 앱만 dmg 로도 나갑니다** ([ADR-0050](adr/0050-store-app-ships-as-a-disk-image.md)).
  ...
  없습니다. 서버는 리눅스라 dmg 를 만들지 못해서 워커가 만듭니다
...
릴리스 한 번을 내는 동안 개발자가 자리를 여러 번 옮깁니다. CLI 로 올리고, 콘솔을
새로고침하며 서명을 기다리고, 실패하면 화면의 안내를 읽어 손으로 고치고, Sparkle 을
쓰는 앱이면 `SUPublicEDKey` 를 복사해 `Info.plist` 에 붙입니다.

**그 자리를 코딩 에이전트가 대신 밟게 합니다.** 서명 실패가 특히 그렇습니다. 실패
화면은 이미 "entitlements 에 `com.apple.security.cs.allow-jit` 를 넣고 다시 올리세요"
까지 적어주는데(ADR-0023), 그것을 읽고 파일을 고치고 다시 올리는 것은 사람이 합니다.
에이전트는 그 셋을 이어서 합니다.
...
스토어 전체를 흔드는 일이라 잘못 부르면 서명 파이프라인이 멈춥니다. 필요해지면 별도
서버로 만들고 자격증명도 나눕니다.

**사람 토큰으로 붙습니다.** 앱별 배포 토큰은 CI 자격증명이라 업로드만 할 수 있고,
그것을 넓히면 러너를 건드릴 수 있는 사람이 피드백과 통계까지 보게 됩니다. 배포 토큰은
그대로 두고 사람 토큰을 따로 둡니다 ([ADR-0060](adr/0060-mcp-connects-as-a-person.md)).

**CLI 의 `alley mcp` 서브커맨드로 냅니다.** 실행 파일을 따로 두면 서명·공증·배포
경로를 하나 더 만들어야 하는데, CLI 는 그것을 이미 갖고 있습니다. 개발자도 받는 것이
하나로 끝나고 설정 파일을 같이 씁니다. Swift 로 쓰니 `AlleyShared` 의 DTO 를 그대로
