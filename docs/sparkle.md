...
> **스토어 앱 자신은 Sparkle 을 쓰지 않습니다.** 앱을 받아 검증하고 설치하는 코드를
> 이미 갖고 있어서 자기 자신을 갈아끼우는 데도 그것을 씁니다
> ([설계 5.5](design.md#55-앱-업데이트)). 스토어 앱을 올리려고 이 문서를 펼쳤다면
> 여기서 덮으세요. 이 키는 워커가 **다른 앱의** 결과물에 서명할 때만 씁니다.
...
앱을 만드는 사람은 **앱 상세 > 자동 업데이트 피드**에서 `SUPublicEDKey` 값을 그대로
복사하면 됩니다 ([ADR-0057](adr/0057-workers-report-the-sparkle-public-key.md)).
워커가 하트비트로 알려준 값입니다. 개인키를 가진 사람을 찾아갈 필요가 없습니다.

그 화면은 쓸 수 있는 상태인지도 함께 말합니다. 키가 없거나, 워커마다 키가 다르거나,
최근 출시본에 서명이 빠져 있으면 그 자리에서 알려줍니다.

아래 `openssl` 명령은 **워커 관리자가 확인하거나 백업할 때** 씁니다. 워커에 별도
명령을 두지 않아서 `openssl` 로 합니다.
